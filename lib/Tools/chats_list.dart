...
class _ChatsListWidgetState extends State<ChatsListWidget> {
            ...
            new Flexible(
              child: new FirebaseAnimatedList(
                  ...
                  itemBuilder:(_, DataSnapshot snapshot, Animation<double> animation, int index){
                    return new ListTile(
                      onTap: (){
                        Navigator.push(context,MaterialPageRoute(builder: (context)=> ChatView(friendListUid: snapshot.key.toString(), currentUserUid: currentUser.getUid(),)));
                      },
  ...
  Future <String> getUserMessageFirebase(String snapshot)async{
    String message;
    await itemRef.child("$snapshot/").limitToLast(1).once().then((DataSnapshot dataSnapshot) {
      ...
      message = snapshotString.substring(startIndex + start.length, endIndex).toString();
    });
    return message;
  }
